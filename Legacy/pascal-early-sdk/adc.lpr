...
const
  ...
  {$ifdef USE_LCD}
  i2c_SDA=4;           // These are GP ports, don't confuse with board pins
  i2c_SCL=5;           // GP4 and GP5 are board pins 6 & 7, work with i2c0Inst
                       // Use GP2 (SDA) and GP3 (SCL) with i2c1Inst.
                       // It must be possible to switch (eg) GP0 & GP1 to work
                       // with i2c0Inst but I have not worked out how.
  {$endif}
...
{$ifdef USE_LCD}
procedure ShowLCD(St1, St2 : string);
begin
    lcd1602_clear();
    lcd1602_set_cursor(0, 0);
    lcd1602_string(copy(St1, 1, 16));   // I am not sure if overrun is a problem or not.
    lcd1602_set_cursor(1, 0);
    lcd1602_string(copy(St2, 1, 16));
end;
{$endif}
...
begin
  //PumpOn := False;
  {$ifdef USE_LCD}
  // I2C0Inst and I2C1Inst are defined in pico_i2c_c.
  // We keep a global Inst in lcd_1602_i2c.Inst so you can use other i2c ....
  lcd_1602_i2c.Inst := i2c0Inst;       // See note about GP ports above.
  i2c_init(lcd_1602_i2c.Inst, 100*1000);
  gpio_set_function(I2C_SDA, TGPIOFunction.GPIO_FUNC_I2C);
  gpio_set_function(I2C_SCL, TGPIOFunction.GPIO_FUNC_I2C);
