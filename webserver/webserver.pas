...
//    FCount : Integer;
    ...
    function DoCommands(FN: string; AResp: TFPHTTPConnectionResponse): boolean;
...
const
    {$ifdef CPUX86_64}                     // test rig
    RPCAP='/home/dbannon/Pascal/SolarHotWater/Raspicapture/raspicapture';
    DATAD='/home/dbannon/Pascal/SolarHotWater/Raspicapture/';
    {$endif}
    {$if defined(CPUARM) or defined(CPUAARCH64)}    // running on a Pi
    RPCAP='/home/dbannon/bin/raspicapture';
    DATAD='/home/dbannon/http/';
    {$endif}

function TTestHTTPServer.DoCommands(FN : string; AResp : TFPHTTPConnectionResponse) : boolean;
begin
    writeln('TTestHTTPServer.DoCommands : FN is ', FN);
    Result := True;
    case FN of
        '/REFRESH' : begin
                //writeln('TTestHTTPServer.DoCommands : REFRESH detected');
                if fileexists(RPCAP) then begin
                    ExecuteProcess(RPCAP, '-p -d ' + DATAD);
                    AResp.Content := '<html><body><h2>OK</h2></p>' + 'might have done that.' + '</p></body></html>';
                end else
                    AResp.Content := '<html><body><h2>Nope !</h2></p>' + 'binary not available.' + '</p></body></html>';
                AResp.SendContent;
                     end;
    else
        result := False
...
begin
    FN:=ARequest.Url;
    if DoCommands(FN, AResponse) then exit;
