...
{ A library unit that knows how to talk to Raspi Ports and to DS18B20 temp sensor
  ...
  History
    2024-12-01  Offset to gpio pin number, see initialisation section below.
    2024-12-01  Alter how we pass a gpio pin number because of above. Now integer.
...
type
  ...
  // WARNING - public functions expect to get passed gpio port numbers WITHOUT the
  // gpio_base offset having been added. Internal functions all work with the offset.
                  ...
                  // Very infrequently, it comes back with a stupid number. Deal with elsewhere.
...
function ControlPort(PortNo : integer; AnAction : TRaspiPortControl) : TRaspiPortStatus;
...
function ReadRaspiPort(Port : integer; out Value : char) : boolean;

function SetRaspiPort(Port : integer; Value : boolean) : boolean;    // True = high
                  ...
                  // Note that this function expects PortNo to NOT HAVE GPIO_Base added yet.
function TestRaspiPort(PortNo : integer; out State : string) : boolean;
...
function Get_RasPi_Utils_Error() : string;
                ...
                // Takes a string containing a port number and converts to an int.
                // Tests and issues a warning if error, result is set to -1
function RasPiPortToInt(Port : string) : integer;

var
    RasPi_Utils_Error : string = '';    // may show previous errors if not cleared.
    GPIO_Base : integer;                // gpio pin numbers may need an offset added, see text initialise section
...
begin
    try
        Result := strtoint(Port);
                ...
                writeln('Cannot convert ', Port, ' to a port number (0..', MaxPortNumb, ')');
                Result := -1;
                end;
    end;
...
begin
    {$I-}
    //writeln('writing ' + Content + ' to file ' + FFname);
    Result := True;
    AssignFile(F, FFName);
...
var
   ...
   //St : string;
begin
//    writeln('TestRaspiPort ', PortNo);
    State := 'no gpio';
    if not DirectoryExists(PIN_PATH) then exit(false);                          // if past here, gpio must exist
    State := '';
    if not DirectoryExists(PIN_PATH + 'gpio' + inttostr(PortNo+GPIO_Base)) then
        exit(true);         // port unexported
    AssignFile(F, PIN_PATH + 'gpio' + inttostr(PortNo + GPIO_Base) +'/direction');
    result := true;
    try try
       reset(F);
       readln(F, State);
...
var
   ...
   Port : string;
begin
   Port := Inttostr(PortNo + GPIO_Base);
    if not TestRaspiPort(PortNo, State) then begin     // Note : GPIO_Base not added here !
    ...
    if AnAction = RaspiPortReset then begin
        // writeln('ControlPort - RaspiPortReset');
        if State = '' then exit(RaspiPortAlready);
        // writeln('ControlPort - Writing');
        WriteToFile(PIN_PATH + 'gpio' + Port + '/' + 'value', '0');
        WriteToFile(PIN_PATH + 'gpio' + Port + '/' + 'direction', 'in');
        writeToFile(PIN_PATH + 'unexport', Port);
        if DirectoryExists(PIN_PATH + 'gpio' + Port) then
            exit(RaspiPortNoSet)
        ...
        if (State = '') then begin
            writeToFile(PIN_PATH + 'export', Port);      // Export will create the target dir
            if WriteToFile(PIN_PATH + 'gpio' + Port + '/' + 'direction', Intended) then
                exit(RaspiPortSuccess)
            else exit(RaspiPortNoSet);
        end;
    end;
    result := RaspiPortWrong;         // its already set and but not set to what we want
end;
...
function ReadRaspiPort(Port : integer; out Value : char) : boolean;   //
begin
    Result := ReadFromFile(PIN_PATH + 'gpio' + inttostr(Port+GPIO_Base) + '/value', Value);
    if not Result then Value := '-';
end;

function SetRaspiPort(Port : string; Value : boolean) : boolean;   // 0 or 1
var
   vCh : char = '0';
begin
   if Value then VCh := '1';
   Result := WriteToFile(PIN_PATH + 'gpio' + Port + '/value', vCh);
end;

function SetRaspiPort(Port : integer; Value : boolean) : boolean;   // 0 or 1
var
   vCh : char = '0';
begin
   if Value then VCh := '1';
   Result := WriteToFile(PIN_PATH + 'gpio' + inttostr(Port + GPIO_Base) + '/value', vCh);
   // Result := SetRaspiPort(inttostr(Port + GPIO_Base), vCh)
end;
        ...
        if FileExists(FFileName) then begin
            ...
            try
                ...
                if St = '' then begin         // its not unusual for a read to fail
                    reset(InFile);            // in almost all cases, a second reading is succeessful
                    readln(InFile, St);
                end;
                closeFile(InFile);
            except
...
begin
    ...
    for Dev in DevArray do
        writeln('Dev : ' + Dev + ' Temp=', ReadDS18B20(Dev));
end;

initialization
    { some time, early-ish, 2024, Debian Bookworm moved to a kernel that does not
      allow GPIO portnumber range to start at zero. Looking at the difference
      between Bullseye and Bookworm, the symlink, /sys/class/gpio/gpiochip0 changes
      to gpiochip512. The '512' being an offset to where the port can really be found.
      This is absolutly NOT widely tested !  Davo, 2024/12/01
    }

    GPIO_Base := -1;
    RasPi_Utils_Error := '';
    if DirectoryExists(PIN_PATH) then begin
        if DirectoryExists(PIN_PATH + 'gpiochip0') then
            GPIO_Base := 0
        else if DirectoryExists(PIN_PATH + 'gpiochip512') then
            GPIO_Base := 512
        else RasPi_Utils_Error := 'Unable to find ' + PIN_PATH + 'gpiochipXXX symlink';
    end else
        RasPi_Utils_Error := 'Unable to open ' + PIN_PATH;
